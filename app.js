...
const airlineData = [
  ...
  },
  {
    ...
    cabin: {
      maxWeight: 8,
      cage: "기내 케이지: 46cm x 28cm x 24cm 이하",
      note: "기내 동반: 생후 15주 이상",
    },
    cargo: {
      maxWeight: 75,
      cage: "화물칸 케이지: 항공사 문의",
    ...
    fees: "항공편별 상이",
    routes: "(해외) 암스테르담",
    contact: "+82 26 105 40 37",
    destinations: uniqIata(["ams"]),
    maxCabinWeight: 8,
    maxCargoWeight: 75,
    breeds: ["small", "medium"],
  },
        ...
        <div class="mx-auto flex w-full flex-col gap-3 rounded-2xl bg-white/90 px-4 py-4 text-ink">
          <div class="flex flex-wrap items-center justify-center gap-3">
            <label class="grid min-w-[160px] flex-1 gap-1 text-xs">
              출발지
              <select id="hero-departure" class="hero-select w-full rounded-xl border border-line px-4 py-2 pr-14 text-sm">
            ...
            <label class="grid min-w-[160px] flex-1 gap-1 text-xs">
              도착지
              <select id="hero-destination-region" class="hero-select w-full rounded-xl border border-line px-4 py-2 pr-14 text-sm">
            ...
            <label class="grid min-w-[160px] flex-1 gap-1 text-xs">
              취항지(공항)
              <select id="hero-destination-airport" class="hero-select w-full rounded-xl border border-line px-4 py-2 pr-14 text-sm" disabled>
                <option value="">—</option>
              </select>
            </label>
            <label class="grid min-w-[160px] flex-1 gap-1 text-xs">
        ...
        <div>
          ...
          <div class="mt-2">
            <div class="flex items-start gap-2 text-xs text-slate-400">
              <p class="flex-1">일반적으로 무게 규정은 캐리어를 포함한 무게를 나타냅니다.</p>
              <p class="flex-shrink-0 text-xs font-semibold tracking-[0.08em] text-slate-500">Last update ${lastUpdateLabel}</p>
            </div>
            <p class="mt-1 text-xs text-slate-400">반려동물 탑승 규정은 달라질 수 있으므로, 각 항공사에 사전 문의 바랍니다.</p>
          </div>
        </div>
      </div>
